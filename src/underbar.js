(function() {
  ...
  _.last = function(array, n) {
    ...
    else return n === undefined ? array[array.length - 1] : array.slice(array.length - n, n + 1)
  };
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(item, key, array){
      result.push(iterator(item, key, array))
    })
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    if (accumulator != undefined){
      ...
      result = collection[0];
      collectionClone.splice(0,1);
    }

    _.each(collectionClone, function(item, key){
      result = iterator(result, collectionClone[key])
    });
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(isTrue, item){
      ...
      if (iterator(item)) return true; else return false;
    }, true);
  };
  ...
  _.some = function(collection, iterator) {
    ...
    if (_.every(collection, function(falseTest){
      if (iterator(falseTest)) return false; else return true;
      })
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      var argKey = JSON.stringify(argArray);
      
      if (!(results.hasOwnProperty(argKey))){
          results[argKey] = func.apply(this, argArray);
      }
  ...
  _.shuffle = function(array) {
    ...
    for (var i = arrayCopy.length; i > 0; i--){
      var getKey = (Math.floor(Math.random() * (i)))
      randomArray.push(arrayCopy[getKey]);
      arrayCopy.splice(getKey, 1);
    }
  ...
  _.zip = function() {
    ...
    for (var i in arguments){
      if (arguments[i].length > longestArg){
        longestArg = arguments[i].length;
      }
    }


    for (var j = 0; j < longestArg; j++){
      var insideArray = [];
      for (var k in arguments){
        insideArray.push(arguments[k][j]);
      }
